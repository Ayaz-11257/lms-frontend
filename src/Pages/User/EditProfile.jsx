...
function EditProfile(){
    ...
    async function onFormSubmit(e){
        e.preventDefault();
        if(!data.fullName){  //!data.avatar bhi dedenmge agar compulsory h toh
            toast.error("Fullname is mandatory");
            return;
        }
        ...
        <HomeLayout>
            <div className="flex items-center justify-center h-[100vh]">
                <form onSubmit={onFormSubmit} className="flex flex-col justify-center gap-5 rounded-lg p-4 text-white w-80 min-h-[26rem] shadow-[0_0_10px_black]" >
                    ...
                    <label className="w-28 h-28 rounded-full m-auto" htmlFor="image_uploads">
                        {data.previewImage ? (
